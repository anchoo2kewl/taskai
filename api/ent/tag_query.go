...
func (_q *TagQuery) QueryTeam() *TeamQuery {
...
func (_q *TagQuery) QueryTaskTags() *TaskTagQuery {
...
func (_q *TagQuery) WithTeam(opts ...func(*TeamQuery)) *TagQuery {
	query := (&TeamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) WithTaskTags(opts ...func(*TaskTagQuery)) *TagQuery {
	query := (&TaskTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withTeam; query != nil {
		if err := _q.loadTeam(ctx, query, nodes, nil,
			func(n *Tag, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTaskTags; query != nil {
		if err := _q.loadTaskTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.TaskTags = []*TaskTag{} },
			func(n *Tag, e *TaskTag) { n.Edges.TaskTags = append(n.Edges.TaskTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Team)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Tag)
	for i := range nodes {
...
func (_q *TagQuery) loadTaskTags(ctx context.Context, query *TaskTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *TaskTag)) error {
	...
	query.Where(predicate.TaskTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.TaskTagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTeam != nil {
			_spec.Node.AddColumnOnce(tag.FieldTeamID)
		}
	}
