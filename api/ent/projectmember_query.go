...
func (_q *ProjectMemberQuery) QueryGranter() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *ProjectMemberQuery) Clone() *ProjectMemberQuery {
	...
	return &ProjectMemberQuery{
		...
		withGranter: _q.withGranter.Clone(),
...
func (_q *ProjectMemberQuery) WithGranter(opts ...func(*UserQuery)) *ProjectMemberQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGranter = query
	return _q
}
...
func (_q *ProjectMemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProjectMember, error) {
	...
	if query := _q.withGranter; query != nil {
		if err := _q.loadGranter(ctx, query, nodes, nil,
			func(n *ProjectMember, e *User) { n.Edges.Granter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectMemberQuery) loadGranter(ctx context.Context, query *UserQuery, nodes []*ProjectMember, init func(*ProjectMember), assign func(*ProjectMember, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*ProjectMember)
	for i := range nodes {
...
func (_q *ProjectMemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withGranter != nil {
			_spec.Node.AddColumnOnce(projectmember.FieldGrantedBy)
		}
	}
